            ...
            List<String> marques = vdao.getAllMarques();
                ...
                <p class=" text-primary-100 mb-6">Explorez notre sélection exclusive de véhicules et gérez vos
                                                ...
                                                <div class="flex flex-wrap gap-3">
                                                    ...
                                                    <a href="MesRendezVous.jsp"
                                                        class="bg-white/20 backdrop-blur rounded-lg p-3 flex items-center hover:bg-white/30 transition-all">
                                                        <i class="fas fa-calendar-check text-white mr-2"></i>
                                                        <div>
                                                            <p class="text-xs text-primary-100">Mes rendez-vous</p>
                                                            <p class="text-xl font-bold text-white">Voir tous</p>
                                                        </div>
                                                    </a>
                                                </div>
                                    </div>
                            ...
                            <div class="mb-12">
                                ...
                                <form method="get" class="bg-white p-6 rounded-xl shadow-md mb-8">
                                    <div class="grid grid-cols-1 md:grid-cols-4 gap-6">
                                        ...
                                        <div>
                                            <label for="db" class="block text-sm font-medium text-gray-700 mb-1">Date de
                                                début</label>
                                            <div class="relative">
                                                <div
                                                ...
                                                <input type="date" id="db" name="dateDebut"
                                                ...
                                                <% for (Location loc : historique) { String statusClass="" ; String
                                                    ...
                                                    <tr class="hover:bg-gray-50">
                                                        ...
                                                        <td class="px-6 py-4 whitespace-nowrap text-right text-sm">
                                                            <% if ("En cours".equalsIgnoreCase(loc.getStatut())) { %>
                                                                <button
                                                                    onclick="openPaymentModal('<%= loc.getId() %>', '<%= loc.getDateDebut() %>')"
                                                                    class="bg-blue-500 hover:bg-blue-600 text-white font-medium px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition duration-300">Payer</button>
                                                                <% } else { %>
                        ...
                        <div id="paymentModal"
                            ...
                            <div class="bg-white rounded-lg p-6 w-96">
                                <h3 class="text-lg font-bold mb-4">Choisissez une méthode de paiement</h3>
                                <div class="flex flex-col gap-4">
                                    <button onclick="openRendezVousModal()"
                                        class="bg-blue-500 hover:bg-blue-600 text-white font-medium px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition duration-300">Rendez-vous
                                        à l'agence</button>
                                    <button onclick="payWithStripe()"
                                        class="bg-green-500 hover:bg-green-600 text-white font-medium px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition duration-300">Payer
                                        avec Stripe</button>
                                </div>
                                <button onclick="closePaymentModal()"
                                    class="mt-4 bg-gray-500 hover:bg-gray-600 text-white font-medium px-4 py-2 rounded-lg shadow-md hover:shadow-lg transition duration-300">Fermer</button>
                            </div>
                        </div>

                        <div id="rendezVousModal"
                            class="hidden fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center">
                            <div class="bg-white rounded-lg p-6 w-96">
                                <h3 class="text-lg font-bold mb-4">Formulaire de rendez-vous</h3>
                                <form id="rendezVousForm" method="post" action="PaymentServlet"
                                    class="space-y-6 bg-white p-6 rounded-lg shadow-lg">
                                    <input type="hidden" name="locationId" id="rendezVousLocationId">
                                    <input type="hidden" name="method" value="agency">
                                    <input type="hidden" name="dateDebut" id="rendezVousDateDebut">
                                    <input type="hidden" name="clientId" value="<%= codeClient %>">
                                    <div id="clientInfoFields" class="space-y-4">
                                        <% if (client.getNumTel()==null || client.getNumTel().isEmpty()) { %>
                                            <div>
                                                <label for="telephone"
                                            ...
                                            <% } %>
                                                <% if (client.getAdresse()==null || client.getAdresse().isEmpty()) { %>
                                                    ...
                                                    <% } %>
                                                        <% if (client.getNcin()==null || client.getNcin().isEmpty()) {
                                                            %>
                                                            <div>
                                                                <label for="ncin"
                                                                    class="block text-sm font-medium text-gray-700">NCIN</label>
                                    ...
                                    <div>
                                        <label for="rendezVousDate" class="block text-sm font-medium text-gray-700">Date
                                            de rendez-vous</label>
                                        <input type="date" id="rendezVousDate" name="rendezVousDate"
                                            class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:ring-primary-500 focus:border-primary-500 sm:text-sm"
                                            required>
                                    </div>
                                    <button type="submit"
                                        class="w-full bg-primary-600 hover:bg-primary-700 text-white font-medium py-3 px-6 rounded-lg shadow-md hover:shadow-lg transition duration-300">Confirmer</button>
                                </form>
                        ...
                        <script>
                            function openPaymentModal(locationId, dateDebut) {
                                console.log('Opening Payment Modal with locationId:', locationId, 'and dateDebut:', dateDebut);
                                if (!locationId || !dateDebut) {
                                    alert('Erreur : locationId ou dateDebut manquant.');
                                    return;
                                }
                                document.getElementById('paymentModal').classList.remove('hidden');
                                document.getElementById('rendezVousLocationId').value = locationId;
                                document.getElementById('rendezVousDateDebut').value = dateDebut;
                            }
                            ...
                            function payWithStripe() {
                                // Initialize Stripe first (do not hardcode keys)
                                const stripePublishableKey = '<%= (System.getenv("STRIPE_PUBLISHABLE_KEY") != null) ? System.getenv("STRIPE_PUBLISHABLE_KEY") : "" %>';
                                if (!stripePublishableKey) {
                                    alert('Stripe n\'est pas configuré. Définissez STRIPE_PUBLISHABLE_KEY.');
                                    return;
                                }
                                const stripe = Stripe(stripePublishableKey);

                                const locationId = document.getElementById('rendezVousLocationId').value;
                                const clientId = '<%= codeClient %>';
                            ...
                            function validateRendezVousDate() {
                                const rendezVousDate = document.getElementById('rendezVousDate').value;
                                const dateDebut = document.getElementById('rendezVousDateDebut').value;

                                if (new Date(rendezVousDate) >= new Date(dateDebut)) {
                                    alert('La date de rendez-vous doit être strictement avant la date de début de location.');
                                    return false;
                                }
                                return true;
                            }

                            document.getElementById('rendezVousForm').onsubmit = function () {
                                const locationId = document.getElementById('rendezVousLocationId').value;
                                const dateDebut = document.getElementById('rendezVousDateDebut').value;
                                console.log('Submitting form with locationId:', locationId, 'and dateDebut:', dateDebut);
                                return validateRendezVousDate();
                            };
                        </script>
                        ...
                        <script>
                            ...
                            function openReservationModal(prefillDebut, prefillFin) {
                                ...
                                // Prefill from the search inputs if available
                            ...
                            function fillReservationDates(form) {
                                const dateDebutInput = document.getElementById('db') || document.getElementById('dateDebut');
                                const dateFinInput = document.getElementById('dateFin');
                                const dateDebut = dateDebutInput ? dateDebutInput.value : '';
                                const dateFin = dateFinInput ? dateFinInput.value : '';

                                // If user already selected dates in the search form, just copy them and submit.
                                if (dateDebut && dateFin) {
                                    if (new Date(dateFin) < new Date(dateDebut)) {
                                        alert('La date de fin doit être après la date de début.');
                                        return false;
                                ...
                                // Otherwise, open a modal to choose dates.
